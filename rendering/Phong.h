...
public:
  ...
  Phong(const Ray& source_ray, glm::vec3 normal) : source_ray_(source_ray), normal_(normal) {};
...
inline glm::vec3 Phong::calc_colour(const Light& light, const Ray& shadow_ray, bool in_shadow) const {
  ...
  float light_dot_normal = std::max(0.f, glm::dot(shadow_ray.direction(), normal_));
  glm::vec3 h_direction = glm::normalize(shadow_ray.direction() - source_ray_.direction());
  float reflection_dot_normal = std::max(0.f, glm::dot(h_direction, normal_));
